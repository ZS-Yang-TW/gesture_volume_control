...
while True:
    ...
    if ret:
        ...
        if result.multi_hand_landmarks:
            for (
                handLms
            ) in result.multi_hand_landmarks:  # 畫出每個手的座標及連線、設定樣式
    ...
    cv2.rectangle(
        img, (50, int(volbar)), (85, 400), (105, 165, 218), cv2.FILLED
    )  # 指示條填滿
    cv2.putText(
        img, f"FPS:{int(fps)}", (30, 50), cv2.FONT_HERSHEY_DUPLEX, 1.5, (49, 68, 52), 3
    )  # fps標籤
    cv2.putText(
        img,
        f"{int(volper)}%",
